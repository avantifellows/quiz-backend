...
async def _get_omr_options_map(question_set_ids: list[str]) -> dict:
    """Return OMR option counts keyed by question_set_id, using cache when available.

    On cache miss, runs the aggregation pipeline against MongoDB and caches
    the result with 1h TTL.
    """
    sorted_ids = sorted(question_set_ids)
    hash_input = ":".join(sorted_ids)
    digest = hashlib.sha256(hash_input.encode()).hexdigest()
    omr_cache_key = cache_key("omr_options", digest)

    cached = await cache_get(omr_cache_key)
    ...
    await cache_set(omr_cache_key, options_count_across_sets, ttl_seconds=3600)
    return options_count_across_sets
...
async def aggregate_and_apply_omr_options(quiz: dict, entity_id: str) -> None:
    ...
    """
    question_set_ids = [question_set["_id"] for question_set in quiz["question_sets"]]

    options_count_across_sets = await _get_omr_options_map(question_set_ids)
